...
CGObject footballw, footballb, tree,tree2, tree3, tree4, flagpost;
CGObject footballw2, footballb2, footballw3, footballb3;
Cloth flag = Cloth(2.5, 1.5, 10, 10);

CGObject *sceneObjects[] = { &ground, &mountain, &tree,&tree2,&tree3,&tree4, &footballw, &footballb, &footballw2, &footballb2, &footballw3, &footballb3, &flagpost };  // include objects that are subject to Physics
																																				  // timing
...
//lighting position
glm::vec3 lightPos(1.0f, 10.0f, 3.0f);

bool rotateCubes = false;

float leaf_axis = -4.2f;
...
struct leafParticle {
	...
	bool operator<(const leafParticle& that) const {
		// Sort in reverse order : far particles drawn first.
		return this->cameradistance > that.cameradistance;
	}
};
const int leafMaxParticles = 50000;
leafParticle leafParticlesContainer[leafMaxParticles];
int LastUsedLeafParticle = 0;

// Finds a Particle in ParticlesContainer which isn't used yet.
// (i.e. life < 0);
int FindUnusedLeafParticle() {

	for (int i = LastUsedLeafParticle; i<leafMaxParticles; i++) {
		if (leafParticlesContainer[i].life < 0) {
			LastUsedParticle = i;
			return i;
		}
	}

	for (int i = 0; i<LastUsedLeafParticle; i++) {
		if (leafParticlesContainer[i].life < 0) {
			LastUsedLeafParticle = i;
			return i;
		}
	}
...
void SortLeafParticles() {
	std::sort(&leafParticlesContainer[0], &leafParticlesContainer[leafMaxParticles]);
}

void initialiseLeafParticles() {
	g_particule_position_size_data = new GLfloat[leafMaxParticles * 4];
	g_particule_color_data = new GLubyte[leafMaxParticles * 4];

	for (int i = 0; i < MaxParticles; i++) {
		leafParticlesContainer[i].life = -1.0f;
		leafParticlesContainer[i].cameradistance = -1.0f;
	}
}
...
void createShaders()
{
	...
	leafParticleID = LoadShaders("../CGcommon/shaders/Particle.vertexshader", "../CGcommon/shaders/Particle.fragmentshader");
...
objl::Mesh mountainMesh()
{
	...
	while (n < nIterations)
	{
		...
		for (unsigned int i = 0; i < indices.size(); i = i + 3)
		{
			...
			float scale = (position1.X - position2.X) / 3;
			...
			if (!findVertex(middlePoints, vertex1.Position, index1))
			{
				...
				vertex1.Position.X += (rand() % 100 * scale / 100.0f);
				vertex1.Position.Z += (rand() % 100 * scale/ 100.0f);
				if (vertex1.Position.Y != 0)
				{
					vertex1.Position.Y += (rand() % 100 * scale / 100.0f);
				}
			...
			if (!findVertex(middlePoints, vertex2.Position, index2))
			{
				...
				vertex2.Position.X += (rand() % 100 * scale / 100.0f);
				vertex2.Position.Z += (rand() % 100 * scale / 100.0f);
				if (vertex2.Position.Y != 0)
				{
					vertex2.Position.Y += (rand() % 100 * scale / 100.0f);
				}
			...
			if (!findVertex(middlePoints, vertex3.Position, index3))
			{
				...
				vertex3.Position.X += (rand() % 100 * scale / 100.0f);
				vertex3.Position.Z += (rand() % 100 * scale / 100.0f);
				if (vertex3.Position.Y != 0)
				{
					vertex3.Position.Y += (rand() % 100 * scale / 100.0f);
				}
...
void createObjects()
{
	...
	glGenVertexArrays(1, &tree2VAO);
	glGenVertexArrays(1, &tree3VAO);
	glGenVertexArrays(1, &tree4VAO);
	...
	// ADD MOUNTAIN
	mountain = loadObjObject(mountainMesh(), true, mountainVAO, false, vec3(5.0f, -0.25f, 5.0f), vec3(1.0f, 1.0f, 1.0f), vec3(0.0f, 1.0f, 0.0f), 0.0f, NULL);
	...
	footballw = loadObjObject(meshes[0], true, footballwVAO, true, vec3(0.0f, 1.0f, 0.0f), vec3(0.2f, 0.2f, 0.2f), vec3(1.0f, 1.0f, 1.0f), 0.65f, NULL);
	...
	footballw2 = loadObjObject(meshes[0], false, footballwVAO, true, vec3(3.0f, 1.0f, 0.0f), vec3(0.2f, 0.2f, 0.2f), vec3(1.0f, 1.0f, 0.0f), 0.6f, NULL);
	...
	footballw3 = loadObjObject(meshes[0], false, footballwVAO, true, vec3(-5.0f, 1.0f, 0.0f), vec3(0.2f, 0.2f, 0.2f), vec3(1.0f, 1.0f, 1.0f), 0.7f, NULL);
	...
	// tree trunk
	const char* treeFileName = "../CGCommon/meshes/Tree/low_poly_tree.obj";
	vector<objl::Mesh> treemeshes = loadMeshes(treeFileName);
	tree = loadObjObject(treemeshes[0], true, treeVAO, false, vec3(-4.2f, -0.0f, -2.0f), vec3(0.01f, 0.01f, 0.01f), vec3(0.160f, 0.82f, 0.45f), 0.0f, NULL);

	// tree leaf
	const char* tree2FileName = "../CGCommon/meshes/Tree2/soccer_ball.obj";
	vector<objl::Mesh> tree2meshes = loadMeshes(tree2FileName);
	tree2 = loadObjObject(tree2meshes[0], true, tree2VAO, false, vec3(-4.7f, 2.2f, -2.0f), vec3(0.03f, 0.03f, 0.03f), vec3(0.139f, 0.69f, 0.19f), 0.0f, NULL);

	// tree leaf2
	const char* tree3FileName = "../CGCommon/meshes/Tree2/sphere.obj";
	vector<objl::Mesh> tree3meshes = loadMeshes(tree3FileName);
	tree3 = loadObjObject(tree3meshes[0], true, tree3VAO, false, vec3(-4.2f, 3.2f, -2.0f), vec3(0.03f, 0.03f, 0.03f), vec3(0.139f, 0.69f, 0.19f), 0.0f, NULL);

	// tree leaf3
	const char* tree4FileName = "../CGCommon/meshes/Tree2/sphere.obj";
	vector<objl::Mesh> tree4meshes = loadMeshes(tree4FileName);
	tree4 = loadObjObject(tree4meshes[0], true, tree4VAO, false, vec3(-3.7f, 2.2f, -2.0f), vec3(0.03f, 0.03f, 0.03f), vec3(0.139f, 0.69f, 0.19f), 0.0f, NULL);


	// add flagpost
	const char* flagPostFileName = "../CGCommon/meshes/Cylinder/cylinder.obj";
	flagpost = loadObjObject(loadMeshes(flagPostFileName)[0], true, flagpostVAO, false, vec3(-5.0f, -0.25f, 5.0f), vec3(1.0f, 1.0f, 1.0f), vec3(0.6f, 0.5f, 0.2f), 0.0f, NULL);
	...
	addToObjectBuffer(&tree2, tree2VAO);
	addToObjectBuffer(&tree3, tree3VAO);
	addToObjectBuffer(&tree4, tree4VAO);
	...
	glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, tree2.startIBO * sizeof(unsigned int), sizeof(unsigned int) * tree2.Mesh.Indices.size(), &tree2.Mesh.Indices[0]);
	glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, tree3.startIBO * sizeof(unsigned int), sizeof(unsigned int) * tree3.Mesh.Indices.size(), &tree3.Mesh.Indices[0]);
	glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, tree4.startIBO * sizeof(unsigned int), sizeof(unsigned int) * tree4.Mesh.Indices.size(), &tree4.Mesh.Indices[0]);
...
void leafParticlesStart() {
	...
	for (int i = 0; i < newparticles; i++) {
		...
		leafParticlesContainer[particleIndex].life = 2.5f; // This particle will live n seconds.
		leafParticlesContainer[particleIndex].pos = glm::vec3(0, -1.30f, -8.0f);
		...
		std::mt19937 rng(0);
		std::uniform_int_distribution<int> gen(0, 2*PI); // uniform, unbiased

		//int r = gen(rng);
		int	phi = gen(rng);
		std::uniform_int_distribution<int> gen2(-1, 1); // uniform, unbiased
		int costheta = gen2(rng);
		std::uniform_int_distribution<int> gen3(0, 1); // uniform, unbiased
		int u = gen3(rng);

		int theta = std::acos(costheta);
		int r = 5 * std::cbrt(u);
		...
		leafParticlesContainer[particleIndex].speed = maindir + randomdir * spread;


		// Very bad way to generate a random color  green : rgba(30, 130, 76, 1)
		leafParticlesContainer[particleIndex].r = 30;//rand() % 256;
		leafParticlesContainer[particleIndex].g = 130; //rand() % 256;
		leafParticlesContainer[particleIndex].b = 76;// rand() % 256;
		leafParticlesContainer[particleIndex].a = (rand() % 256) ;

		leafParticlesContainer[particleIndex].size =  0.05f;

	}

	// Simulate all particles
	int ParticlesCount = 0;
	for (int i = 0; i < leafMaxParticles; i++) {

		leafParticle& p = leafParticlesContainer[i]; // shortcut

		if (p.life > 0.0f) {
			//printf("%.6f", Physics::forces(1));
			// Decrease life
			p.life -= deltaTime;
			if (p.life > 0.0f) {

				// Simulate simple physics : gravity only, no collisions
				p.speed += glm::vec3(0.0f, -9.8, 0.0f) * (float)deltaTime * 0.005f;
...
*/
	...
	bool plus = true;
	/*leaf_axis = leaf_axis - 0.005;
	if (leaf_axis < -5.2f) {
		leaf_axis = -3.2f;
	}*/
	
	glm::vec3 leavesPos(leaf_axis, 5.0f, 6.0f);
	//root camera
	glm::mat4 local_leaves(1.0f);
	local_leaves = glm::translate(local_leaves, leavesPos);
	glm::mat4 glob_leaves = local_leaves;
	glUniformMatrix4fv(model_waterparticle, 1, GL_FALSE, &glob_leaves[0][0]);
...
void display()
{
	...
	for (int i = 0; i < newparticles; i++) {
		...
		ParticlesContainer[particleIndex].pos = glm::vec3(0, -1.30f, -8.0f);
...
int main(void) {
	...
	glDeleteVertexArrays(1, &tree2VAO);
	glDeleteVertexArrays(1, &tree3VAO);
	glDeleteVertexArrays(1, &tree4VAO);
